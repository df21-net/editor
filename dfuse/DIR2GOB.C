...
main()
{
printf("DIR2GOB "DIR2GOB_VERSION" (c) Yves BORCKMANS 1995\n\n");

/* Check number of arguments */
if(_argc != 3)
  {
   printf("Usage is DIR2GOB directory name\n");
   printf("Where directory is a valid directory, without trailing \\.\n");
   return(ERR_CMDLINE);
  }

strcpy(gox, _argv[1]);
sprintf(buf, "dir /B %s\\*.* > dir2gob.$$$", gox);
system(buf);

strcpy(fnamegom  , "dir2gob.$$$");
...
while(fgets(buf, 255, fgom) != NULL)
 {
  ...
  if(strlen(buf) > 0)
   {
    strcpy(fnamef, gox);
    strcat(fnamef, strupr(buf));
    if((ff=open(fnamef , O_BINARY | O_RDONLY)) != -1)
     {
      counter++;
      /* remove spaces from master dir entry */
      for(i=0; i<13; i++) if(buf[i] == ' ') buf[i] = '\0';
      strcpy(is.NAME, buf);
      is.IX  = tell(fgob);
