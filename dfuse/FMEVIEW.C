...
void handle_ini()
{
 if(fexists(DFUSE_INI))
  {
   ReadIniString("FMEVIEW-BGI", ".BGI", "", BGIdriver, 10, DFUSE_INI);
   BGImode = ReadIniInt("FMEVIEW-BGI", "MODE", 30000, DFUSE_INI);
   ...
   if((strcmp(BGIdriver,"")==0) || (BGImode == 30000))
    {
     ...
     if((strcmp(BGIdriver,""))==0 || (BGImode == 30000))
      {
       printf("FMEVIEW can't find coherent values for .BGI and MODE !\n");
       printf("Please reconfigure the [DEFAULT-BGI] or [FMEVIEW-BGI] sections of DFUSE.INI .\n\n");
       exit(ERR_GRAPHIC);
      }
    }
   ...
   if(_argc != 3)
    {
     ReadIniString("FMEVIEW-PAL", ".PAL",  "",  PALfile, 100, DFUSE_INI);
     if(strcmp(PALfile,"")==0)
      {
       ReadIniString("DEFAULT-PAL", ".PAL",  "",  PALfile, 100, DFUSE_INI);
       if(strcmp(PALfile,"")==0)
        {
         printf("FMEVIEW can't find coherent values for .PAL !\n");
         printf("Please reconfigure the [DEFAULT-PAL] or [FMEVIEW-PAL] sections of DFUSE.INI .\n\n");
         exit(ERR_GRAPHIC);
        }
      }
     strcat(PALfile, ".pal");
  ...
  {
   printf("FMEVIEW can't find "DFUSE_INI"\n");
...
main()
{
 ...
 printf("\n\nFMEVIEW "FMEVIEW_VERSION" (c) Yves BORCKMANS 1995.\n\n");
 /* Check number of arguments */
 if((_argc != 2) && (_argc != 3))
  {
   printf("Usage is  FMEVIEW fmefile [palfile]\n");
 ...
 /* ie BGI to use and in which mode AND palette file to use AND show palette ? */
 handle_ini();
 ...
 read(f1, &FME1, 32);
 lseek(f1, FME1.header2, SEEK_SET);
 read(f1, &BMH, 24);
