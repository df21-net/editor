...
uses
  ...
  ComCtrls, Tabnotbk, Level, GlobalVars;
      ...
      begin
        ...
        MM_SC: begin
                 for m := 0 to SC_MULTIS.Count - 1 do
                  begin
                   TheSector := TheLevel.Sectors[StrToInt(Copy(SC_MULTIS[m],1,4))];
                   for j := 0 to TheSector.Walls.Count - 1 do
                     begin
                       TheWall := TheSector.Walls[j];
                       LVertex := TheSector.Vertices[TheWall.V1];
        ...
        MM_VX: begin
                 ...
                 for m := 0 to VX_MULTIS.Count - 1 do
                  begin
                   TheSector := TheLevel.Sectors[StrToInt(Copy(VX_MULTIS[m],1,4))];
                   TheVertex := TheSector.Vertices[StrToInt(Copy(VX_MULTIS[m],5,4))];
        ...
        MM_OB: begin
                ...
                for m := 0 to OB_MULTIS.Count - 1 do
                  begin
                   TheObject := TheLevel.Objects[StrToInt(Copy(OB_MULTIS[m],5,4))];
                   if IsPtVisible(TheObject.X, TheObject.Z) then
                    if true {TheObject.Special = 1} then {GENERATOR : draw a 'G'}
                     begin
      ...
      MM_OB: begin
              ...
              for i := 0 to TheLevel.Objects.Count - 1 do
                begin
                  ...
                  if (TheLayer = LAYER) or (TheLayer = 9999) then
                   begin
                    Pen.Color := clWhite; {TheObject.Col;}
                    if IsPtVisible(TheObject.X, TheObject.Z) then
                    if {TheObject.Special = 1} false then {GENERATOR : draw a 'G'}
                     begin
                      MoveTo(M2SX(TheObject.X)+Dimension, M2SZ(TheObject.Z)-Dimension);
                      LineTo(M2SX(TheObject.X)-Dimension, M2SZ(TheObject.Z)-Dimension);
                      LineTo(M2SX(TheObject.X)-Dimension, M2SZ(TheObject.Z)+Dimension);
...
begin
  ...
  BaseDir    := Copy(ExtractFilePath(ParamStr(0)),1, Length(ExtractFilePath(ParamStr(0)))-1);
  LEVELloaded      := FALSE;
  ChkCmdLine       := TRUE;
...
begin
  if LEVELLoaded and MODIFIED then
   ...
   begin
    if LEVELLoaded then TheLevel.Free;
    CanClose := TRUE;
   end;
end;
