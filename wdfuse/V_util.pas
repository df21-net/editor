...
function OffsetHugePointer(HugePtr: Pointer; HugeOffset: LongInt): Pointer; assembler;
asm
  mov ax, HugeOffset.Word[0]
  mov dx, HugeOffset.Word[2]
     ...
     IF BMH.Compressed = 0 THEN
       if (BMH.SizeX <> 1) or ((BMH.SizeX = 1) and (BMH.SizeY = 1)) then
         begin
           ...
           NODELPHI_hread(gf, GlobalP, BMH.dataSize);
           FileClose(gf);

           {
           hdcMem     := CreateCompatibleDC(BMHDC);
           gdiBitmap  := CreateCompatibleBitmap(BMHDC, BMH.SizeX, BMH.SizeY);
           oldBitmap  := SelectObject(hdcMem, gdiBitmap);
           }
           ...
           for i := 0 to BMH.SizeY-1 do
            begin
             for j := 0 to BMH.SizeX-1 do
              begin
               SetPixel(BMHDC {hdcMem}, j, BMH.SizeY-i-1 ,
                  $01000000 + PByte(OffsetHugePointer(GlobalP, Bidon * BMH.SizeY * j + i))^);
              end;

            {
            BitBlt(BMHDC, 0, 0, BMH.SizeX, BMH.SizeY, hdcMem, 0, 0, SRCCOPY);
            ...
            DeleteDC(hdcMem);
             ...
             begin
              ...
              Info.Lines.Add('idem X      : ' + IntToStr(BMH.idemX));
              Info.Lines.Add('idem Y      : ' + IntToStr(BMH.idemY));
           ...
           with TheRESOURCE do
            ...
            for k := 1 to nbbm do
             begin
               ...
               Info.Lines.Add('idem X      : ' + IntToStr(BMSUBH.idemX));
               Info.Lines.Add('idem Y      : ' + IntToStr(BMSUBH.idemY));
               ...
               NODELPHI_hread(gf, GlobalP, BMSUBH.DataSize);
               for i := 0 to BMSUBH.SizeY do
                 for j := 0 to BMSUBH.SizeX do
                  begin
                   SetPixel(BMHDC, j+(k-1)*(10+BMSUBH.SizeX), BMSUBH.SizeY-i-1 ,
                            $1000000 + PByte(OffsetHugePointer(GlobalP, Bidon * i + BMSUBH.SizeY*j))^);
                  end;
             end;
             Info.Lines.EndUpdate;
             FileClose(gf);
     ...
     ELSE
       BEGIN
        {Compressed}
         ...
         Image.Picture.Bitmap.Width  := BMH.SizeX;
         BMHDC  := Image.Picture.Bitmap.Canvas.Handle;
         NODELPHI_hread(gf, GlobalP, BMH.dataSize + Bidon * BMH.SizeX * 4);

         for i := 0 to BMH.SizeX-1 do
          begin
           col_start := PLong(OffsetHugePointer(GlobalP, BMH.DataSize + Bidon * i * 4))^;
           if i < BMH.SizeX-1 then
            col_end   := PLong(OffsetHugePointer(GlobalP, BMH.DataSize + Bidon * (i+1) * 4))^
           else
           ...
           while (col_start+j < col_end) do
            begin
             if (PByte(OffsetHugePointer(GlobalP, col_start+j))^ >= 128) then
              begin
               rle := PByte(OffsetHugePointer(GlobalP, col_start+j))^ - 128;

               if BMH.Compressed = 1 then {RLE}
                begin
                 {position on color byte}
                 Inc(j);
                 {store it to avoid the OffsetHugePointer call rle times !}
                 colorbyte := PByte(OffsetHugePointer(GlobalP, col_start+j))^;
                 for m :=0 to rle-1 do
                  begin
                   SetPixel(BMHDC, i, BMH.SizeY-k-1 , $1000000 + colorbyte);
                   Inc(k);
                  end;
                 Inc(j);
                end
                ...
                begin
                 for m :=0 to rle-1 do
                  begin
                   SetPixel(BMHDC, i, BMH.SizeY-k-1 , $1000000);
                   Inc(k);
                  end;
                 Inc(j);
                end;
              ...
              begin
               n := PByte(OffsetHugePointer(GlobalP, col_start+j))^;
               Inc(j);
               for m:=0 to n-1 do
                begin
                 SetPixel(BMHDC, i, BMH.SizeY-k-1 ,
                   $1000000 + PByte(OffsetHugePointer(GlobalP, col_start+j))^);
                 Inc(k);
                 Inc(j);
                end;
              end;
...
begin
 ...
 if GlobalH <> 0 then
   begin
     ...
     NODELPHI_hread(gf, GlobalP, 140000);
     FileClose(gf);
     ...
     if BMH.Compressed = 0 then
       ...
       for i := 0 to BMH.SizeX-1 do
        begin
         for j := 0 to BMH.SizeY-1 do
          begin
           if FME1.Flip = 0 then
            SetPixel(BMHDC, i, BMH.SizeY-j-1 ,
              $1000000 + PByte(OffsetHugePointer(GlobalP, Bidon * BMH.SizeY * i + j))^)
           else
            SetPixel(BMHDC, BMH.SizeX-i-1, BMH.SizeY-j-1 ,
              $1000000 + PByte(OffsetHugePointer(GlobalP, Bidon * BMH.SizeY * i + j))^);
          end;
        end;
      end
      ...
      begin
       ...
       for i := 0 to BMH.SizeX-1 do
        begin
         col_start := PLong(OffsetHugePointer(GlobalP, Bidon * i * 4))^ - 24;
         if i < BMH.SizeX-1 then
          col_end   := PLong(OffsetHugePointer(GlobalP, Bidon * (i+1) * 4))^ - 24
         else
             ...
             for m :=0 to rle-1 do
              begin
               if FME1.Flip = 0 then
                ...
                SetPixel(BMHDC, BMH.SizeX-i-1, BMH.SizeY-k-1 , $1000000);
               Inc(k);
              end;
             Inc(j);
            end
             ...
             for m:=0 to n-1 do
              begin
               if FME1.Flip = 0 then
                ...
                SetPixel(BMHDC, BMH.SizeX-i-1, BMH.SizeY-k-1 ,
                 $1000000 + PByte(OffsetHugePointer(GlobalP, col_start+j))^);
               Inc(k);
               Inc(j);
              end;
...
begin
 ...
 NODELPHI_hread(gf, GlobalP, DataSize);
 FileClose(gf);
 curpos := 0;
 ...
 while TRUE do
  begin
   Aline.Size := PInt(OffsetHugePointer(GlobalP, curpos))^;
   Inc(curpos, 2);
   ...
   {read X and Y pos}
   Aline.PosX := PInt(OffsetHugePointer(GlobalP, curpos))^;
   Inc(curpos, 2);
   Aline.PosY := PInt(OffsetHugePointer(GlobalP, curpos))^;
   Inc(curpos, 2);
   ...
   if (ALine.Size and 1) = 0 then
    begin
     ...
     for i := 0 to (ALine.Size shr 1) - 1 do
      begin
       SetPixel(DHDC, ALine.PosX{-AHeader.OffsX} + i, Aline.PosY{-AHeader.OffsY},
                      $1000000 + PByte(OffsetHugePointer(GlobalP, curpos))^);
       Inc(curpos);
      end;
    end
   else
    begin
     ...
     while (rle < ALine.Size shr 1) do
      begin
       n := PByte(OffsetHugePointer(GlobalP, curpos))^;
       Inc(curpos);
       ...
       if (n and 1) = 0 then
        begin
         ...
         for i := 0 to (n shr 1) - 1 do
          begin
           SetPixel(DHDC, ALine.PosX{-AHeader.OffsX} + skip, Aline.PosY{-AHeader.OffsY},
                      $1000000 + PByte(OffsetHugePointer(GlobalP, curpos))^);
           Inc(curpos);
           Inc(skip);
          end;
...
begin
 ...
 PalDC := Image.Picture.Bitmap.Canvas.Handle; {pq 2* ?}
 ...
 with Image.Picture.Bitmap.Canvas do
  begin
   ...
   for i := 0 to 255 do
    for j := 0 to 31 do
     begin
      Brush.Color := $1000000 + PByte(OffsetHugePointer(GlobalP, Bidon * i + (31 - j) * 256))^;
...
begin
 ...
 {
 hdc          := CreateCompatibleDC(PalDC);
 Bitmap       := CreateCompatibleBitmap(PalDC, rs * 16, rs * 16);
 oldBitmap    := SelectObject(hdc, Bitmap);
 GrayPen      := CreatePen(PS_SOLID, 1, clGray);
 oldPen       := SelectObject(hdc, GrayPen);
 oldPalette   := SelectPalette(hdc, HPALPalette, FALSE);
 RealizePalette(hdc);
 PatBlt(hdc, 0, 0, 400, 400, BLACKNESS);
 }
   ...
   for i := 0 to 15 do
    for j := 0 to 15 do
     begin
      ...
      {
      SetRect(TheRect, rs * j, rs * i, rs * j + rs - 1, rs * i + rs - 1);
      TheBrush := CreateSolidBrush($01000000 + Byte(i * 16 + j));
      oldBrush := SelectObject(hdc, TheBrush);
      setpixel(hdc, i,j, $01000000 + Byte(i * 16 + j));
      FillRect(hdc, TheRect, TheBrush);
 ...
 {
 BitBlt(PalDC, 0, 0, 400, 400, hdc, 0, 0, SRCCOPY);
...
var i,j,k,m,n    : Integer;
    ...
    oldPen       : HPen;
begin
 {Extract from gob, or copy file, so we always work with
  the WDFUSEdir + '\WDFGRAPH\WDF$$$$$.3DO' file }
 DeleteFile(WDFUSEdir + '\WDFGRAPH\' + 'WDF$$$$$.3DO');
 if GraphFile[1] = '[' then
  begin
  ...
  if not FileExists(WDFUSEdir + '\WDFGRAPH\' + 'WDF$$$$$.3DO') then
    ...
    begin
     ...
     hdc       := CreateCompatibleDC(BMHDC);
     Bitmap    := CreateCompatibleBitmap(BMHDC, 400, 400);
     oldBitmap := SelectObject(hdc, Bitmap);
     LimePen   := CreatePen(PS_SOLID, 1, clLime);
     oldPen    := SelectObject(hdc, LimePen);
     SetTextColor(hdc, clLime);
     SetBkColor(hdc, clBlack);

     PatBlt(hdc, 0, 0, 400, 400, BLACKNESS);
     {hdc := Image.Picture.Bitmap.Canvas.Handle;}

     TextOut(hdc,   1,  1, 'X-Y', 3);
     TextOut(hdc,   1,201, 'Z-Y', 3);
     TextOut(hdc, 201,  1, 'X-Z', 3);
      ...
      BEGIN
       ...
       for j := 0 to The3DOObject.Triangles.Count - 1 do
        begin
         ...
         with The3DOTriangle do
          begin
           MoveTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Y));

           MoveTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Z));
           LineTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).Z));
           LineTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Z));
           LineTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Z));

           MoveTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Z),
                       300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).Z),
                       300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[B]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Z),
                       300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Z),
                       300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[A]).Y));
        ...
        begin
         ...
         with The3DOQuad do
          begin
           ...
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[D]).X),
           ...
           LineTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).X),
                       100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Z));
           LineTo(hdc, 300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[D]).X),
           ...
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Z),
                       300+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[C]).Y));
           LineTo(hdc, 100+Round(Scale3DO*T3DO_Vertex(The3DOObject.Vertices.Objects[D]).Z),
     ...
     BitBlt(BMHDC, 0, 0, 400, 400, hdc, 0, 0, SRCCOPY);

     SelectObject(hdc, oldPen);
     DeleteObject(LimePen);
     SelectObject(hdc, oldBitmap);
     DeleteObject(Bitmap);
     DeleteDC(hdc);
