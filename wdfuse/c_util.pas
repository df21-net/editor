...
for i := 0 to 255 do
 ...
 {This saves the last conversion array to disk. Useful to generate
  Duke cnv files :-)}
 {
 i := FileCreate('c:\wdfduke.cnv');
 FileWrite(i, quantize_array, 256);
 FileWrite(i, quantize_array, 1);
 FileClose(i);
 }
end;
...
begin
 ...
 IF BMH.Compressed = 0 THEN
  if (BMH.SizeX <> 1) or ((BMH.SizeX = 1) and (BMH.SizeY = 1)) then
   begin
    ...
    NODELPHI_hread(fin, GPBM, BMH.dataSize);
    for i := 0 to BMH.SizeY-1 do
     begin
      for j := 0 to BMH.SizeX-1 do
       begin
         PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * i + j))^ :=
          PByte(OffsetHugePointer(GPBM, Bidon * BMH.SizeY * j + i))^;
       end;
     end;
    go := TRUE;
   end
 ...
 ELSE
  BEGIN
   ...
   NODELPHI_hread(fin, GPBM, BMH.dataSize + Bidon * BMH.SizeX * 4);

   for i := 0 to BMH.SizeX-1 do
    begin
     col_start := PLong(OffsetHugePointer(GPBM, BMH.DataSize + Bidon * i * 4))^;
     if i < BMH.SizeX-1 then
      col_end   := PLong(OffsetHugePointer(GPBM, BMH.DataSize + Bidon * (i+1) * 4))^
     else
     ...
     while (col_start+j < col_end) do
      begin
       if (PByte(OffsetHugePointer(GPBM, col_start+j))^ >= 128) then
        begin
         rle := PByte(OffsetHugePointer(GPBM, col_start+j))^ - 128;
         ...
         if BMH.Compressed = 1 then {RLE}
          begin
           ...
           colorbyte := PByte(OffsetHugePointer(GPBM, col_start+j))^;
           for m :=0 to rle-1 do
            begin
             PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * k + i))^ :=
              colorbyte;
             Inc(k);
            end;
           Inc(j);
        ...
        begin
         n := PByte(OffsetHugePointer(GPBM, col_start+j))^;
         Inc(j);
         for m:=0 to n-1 do
          begin
           PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * k + i))^ :=
             PByte(OffsetHugePointer(GPBM, col_start+j))^;
           Inc(k);
           Inc(j);
          end;
        end;
 ...
 NODELPHI_hwrite(fout, GPBMP, bmp_counter);
 FileClose(fout);
...
begin
 ...
 FileRead(fin, BMH, 24);
 if BMH.Compressed = 0 then
  NODELPHI_hread(fin, GPBM, Bidon * (BMH.SizeX + bmp_align) * BMH.SizeY)
 else
  NODELPHI_hread(fin, GPBM, BMH.DataSize - 24);
 FileClose(fin);
 ...
 if BMH.Compressed = 0 then
    ...
    for i := 0 to BMH.SizeX-1 do
     begin
      for j := 0 to BMH.SizeY-1 do
       begin
        if FME1.Flip = 0 then
         PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + i))^ :=
           PByte(OffsetHugePointer(GPBM, Bidon * BMH.SizeY * i + j))^
        else
         PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + BMH.SizeX-i))^ :=
           PByte(OffsetHugePointer(GPBM, Bidon * BMH.SizeY * i + j))^;
       end;
     end;
   end
   ...
   begin
    ...
    for i := 0 to BMH.SizeX-1 do
     begin
      col_start := PLong(OffsetHugePointer(GPBM, Bidon * i * 4))^ - 24;
      if i < BMH.SizeX-1 then
       col_end   := PLong(OffsetHugePointer(GPBM, Bidon * (i+1) * 4))^ - 24
      else
          ...
          for m :=0 to rle-1 do
           begin
            ...
            if FME1.Flip = 0 then
             ...
             PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * k + BMH.SizeX-i))^ :=
               0;
            }
            Inc(k);
           end;
...
begin
 ...
 NODELPHI_hread(fin, GPDELT, DataSize);
 FileClose(fin);
 curpos := 0;
 ...
 while TRUE do
  begin
   Aline.Size := PInt(OffsetHugePointer(GPDELT, curpos))^;
   Inc(curpos, 2);
   ...
   {read X and Y pos}
   Aline.PosX := PInt(OffsetHugePointer(GPDELT, curpos))^;
   Inc(curpos, 2);
   Aline.PosY := PInt(OffsetHugePointer(GPDELT, curpos))^;
   Inc(curpos, 2);
   ...
   if (ALine.Size and 1) = 0 then
    begin
     ...
     for i := 0 to (ALine.Size shr 1) - 1 do
      begin
       PByte(OffsetHugePointer(GPBMP,
         bmp_counter - Bidon * (AHeader.SizeX + bmp_align) * ALine.PosY + ALine.PosX + i))^ :=
        PByte(OffsetHugePointer(GPDELT, curpos))^;
       Inc(curpos);
      end;
    end
   else
    begin
     ...
     while (rle < ALine.Size shr 1) do
      begin
       n := PByte(OffsetHugePointer(GPDELT, curpos))^;
       Inc(curpos);
       ...
       if (n and 1) = 0 then
        begin
         ...
         for i := 0 to (n shr 1) - 1 do
          begin
           PByte(OffsetHugePointer(GPBMP,
             bmp_counter - Bidon * (AHeader.SizeX + bmp_align) * ALine.PosY + ALine.PosX + skip))^ :=
            PByte(OffsetHugePointer(GPDELT, curpos))^;
           Inc(curpos);
           Inc(skip);
          end;
...
begin
 ...
 NODELPHI_hread(fin, GPBMP, BMP_IH.biSizeImage);
 FileClose(fin);
 ...
 for j := 0 to BMH.SizeY-1 do
  begin
   for i := 0 to BMH.SizeX-1 do
    begin
     PByte(OffsetHugePointer(GPBM, Bidon * BMH.SizeY * i + j))^ :=
       quantize[PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + i))^];
    end;
  end;

 fout := FileCreate(BMname);
 FileWrite(fout, BMH,  32);
 NODELPHI_hwrite(fout, GPBM, Bidon * BMH.SizeX * BMH.SizeY);
 FileClose(fout);
...
begin
 ...
 for i := 0 to BMH.SizeX-1 do
  begin
   j := 0;
   PLong(OffsetHugePointer(GPBM, Bidon * i * 4))^ := DataPos + 24;
   while j < BMH.SizeY do
    BEGIN
     len := 0;
     if PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + i))^ = 0 then
      begin
       while (len < 127) and
             (j < BMH.SizeY) and
             (PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + i))^ = 0) do
        begin
         Inc(len);
         Inc(j);
        end;
       PByte(OffsetHugePointer(GPBM, DataPos))^ := len + 128;
       Inc(DataPos);
      end
     else
      begin
       while (len < 127) and
        ...
        begin
         Inc(len);
         obuf[len] := PByte(OffsetHugePointer(GPBMP, Bidon * (BMH.SizeX + bmp_align) * j + i))^;
         Inc(j);
        end;
       obuf[0] := len;
       for k := 0 to len do
        begin
         if k = 0 then
          PByte(OffsetHugePointer(GPBM, DataPos))^ := obuf[k]
         else
          PByte(OffsetHugePointer(GPBM, DataPos))^ := quantize[obuf[k]];
         Inc(DataPos);
        end;
      end;
 ...
 FileWrite(fout, FME,  32);
 FileWrite(fout, BMH,  24);
 NODELPHI_hwrite(fout, GPBM, BMH.DataSize);
 FileClose(fout);
 ...
 for j := 0 to DH.SizeY-1 do
  begin
   ...
   while i < DH.SizeX do
    begin
     {skip background}
     if PByte(OffsetHugePointer(GPBMP, Bidon * (DH.SizeX+bmp_align) * (DH.SizeY-j-1) + i))^ = 0 then
      Inc(i)
     else
      begin
       curstart := i;
       ...
       while (len < 127) and
             (i < DH.SizeX) and
             (PByte(OffsetHugePointer(GPBMP, Bidon * (DH.SizeX+bmp_align) * (DH.SizeY-j-1) + i))^ <> 0) do
        begin
         Inc(len);
         Inc(i);
        end;
        ...
        while iptr < len do
         begin
          v := PByte(OffsetHugePointer(GPBMP, Bidon * (DH.SizeX+bmp_align) * (DH.SizeY-j-1) + curstart + iptr))^;
          n := 1;
          Inc(iptr);
          while (v = PByte(OffsetHugePointer(GPBMP, Bidon * (DH.SizeX+bmp_align) * (DH.SizeY-j-1) + curstart + iptr))^)
                and (iptr < len) do
           begin
            Inc(n);
            Inc(iptr);
         ...
         if optr < len then
          ...
          begin
           ...
           for optr := 0 to len - 1 do
            obuf[optr] := PByte(OffsetHugePointer(GPBMP,
              Bidon * (DH.SizeX+bmp_align) * (DH.SizeY-j-1) + curstart + optr))^;
